...
export class ScientificChart{
  ...
  draw(){
    ...
    const left=kind==='history'?96:60,right=28,top=16,bottom=40,w=width-left-right,h=height-top-bottom;
    ...
    const unit=document.createElement('p');unit.className='chart-unit';
    unit.textContent=kind==='history'?`Clean objective · lower is better${this.log?' · log scale':''}`:'Fraction of problems solved · higher is better';
    this.host.append(unit);
    ...
    const maxTicks=width<400?3:width<560?4:8;
    if(xticks.length>maxTicks)xticks=xticks.filter((_,i)=>i%Math.ceil(xticks.length/maxTicks)===0);
    ...
    const axis=document.createElement('p');axis.className='chart-axis';
    axis.textContent=kind==='performance'?'Performance ratio':kind==='data'?'Objective evaluations / (dimension + 1)':'Objective evaluations';
